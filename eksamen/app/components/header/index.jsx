...
export default function Header() {
    ...
    return (
        <>
            ...
            <h1 className="absolute mt-4 left-20 text-2xl capitalize">{Headline}</h1>
            ...
            <nav  className="col-span-3 row-1 flex absoulte bg-white z-1000 justify-end w-full content-normal">            
                ...
                <ul className={showMenu ? " h-[100vh] flex flex-col bg-white w-full mt-20 text-center items-center text-2xl gap-4" : "hidden" }>
                    <li>
                        <Link className={` ${isActive("/", pathname)}`} href="/" onClick={() => setShowMenu(false)}>Home</Link>
                    </li>
                    <li>
                        <Link className={` ${isActive("/aktiviteter", pathname)}`} href="/popular-classes" onClick={() => setShowMenu(false)}>Popular Classes</Link>
                    </li>
                    <li>
                        <Link className={` ${isActive("/search", pathname)}`} href="/search" onClick={() => setShowMenu(false)}>Search</Link>
                    </li>
                    <li>
                        <Link className={` ${isActive("/profile", pathname)}`} href="/profile" onClick={() => setShowMenu(false)}>My Profile</Link> {/* Fjernet: { profilMenuText } - Skift mellem "Profil" og "Log ind" baseret på loginstatus */}
                    </li>
                    <li>
                        <Link className={` ${isActive("/profile", pathname)}`} href="/profile onClick={() => setShowMenu(false)}">Log Out</Link> 
                    </li>
                </ul>
            </nav>
        </>
